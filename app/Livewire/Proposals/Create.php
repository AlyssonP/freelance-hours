...
class Create extends Component
{
    ...
    public function arrangePositions($proposal) {
        $query = DB::select("
        ...
        $outherProposal = collect($query)->where("position","=", $position->newPosition)->first();

        if ($outherProposal) {
            $proposal->update(["position_status" => "up"]);
            Proposal::query()->where("id", "=", $outherProposal->id)->update(["position_status" => "down"]);
        }
